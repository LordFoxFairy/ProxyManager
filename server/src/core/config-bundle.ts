...
import { listProviders, upsertProvider, type Provider } from './providers.js';
import { listGroups, upsertGroup, type ProxyGroup } from './groups.js';
import { listRules, listRuleProviders, upsertRule, upsertRuleProvider, type RoutingRule, type RuleProvider } from './rules.js';
...
export function importConfigBundle(input: unknown): ConfigBundle { if (!input || typeof input !== 'object') throw new Error('配置文件格式无效'); const row = input as Partial<ConfigBundle>; if (row.format !== 'proxymanager-config' || row.version !== 1 || !row.runtime || !Array.isArray(row.providers) || !Array.isArray(row.groups) || !Array.isArray(row.rules) || !Array.isArray(row.ruleProviders)) throw new Error('不支持的 ProxyManager 配置版本'); updateRuntimeConfig(row.runtime); row.providers.forEach((item) => upsertProvider(item)); row.groups.forEach((item) => upsertGroup(item)); row.rules.forEach((item) => upsertRule(item)); row.ruleProviders.forEach((item) => upsertRuleProvider(item)); return exportConfigBundle(); }
