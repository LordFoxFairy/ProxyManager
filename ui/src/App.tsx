...
function ProxyManagerApp() {
  ...
  const poolProps = {
    ...
    setOnlyExitIp: model.setOnlyExitIp,
  ...
  return (
    <div className="app">
      ...
      <main className={`main page-${page}${page === 'resources' ? ` resource-view-${model.resourceView}` : ''}`}>
        ...
        {page === 'rules' && <RulesPage rules={model.rules} groups={model.groups} ruleProviders={model.ruleProviders} onSave={model.saveRoutingRule} onPatch={model.patchRoutingRule} onRemove={model.removeRoutingRule} onTest={model.testRoutingRule} onSaveProvider={model.saveRuleProviderState} onPatchProvider={model.patchRuleProviderState} onRemoveProvider={model.removeRuleProviderState} onRefreshProvider={model.refreshRuleProviderState} />}
        {page === 'routing' && gateway && model.runtimeStatus && model.runtimeConfig && <GatewayPage gateway={gateway} runtimeStatus={model.runtimeStatus} runtimeConfig={model.runtimeConfig} systemProxyActual={model.systemProxyActual} tunActual={model.tunActual} onSaveRuntime={model.saveRuntime} onRuntimeAction={model.runRuntimeAction} onRollbackRuntime={model.rollbackRuntimeConfig} onExportConfig={model.exportConfiguration} onImportConfig={model.importConfiguration} desktopUpdate={model.desktopUpdate} desktopUpdateBusy={model.desktopUpdateBusy} onCheckDesktopUpdate={model.checkDesktopUpdate} onInstallDesktopUpdate={model.installDesktopUpdate} copied={model.copied} routingSaving={model.gatewayRoutingSaving} routingError={model.gatewayRoutingError} profileOptions={model.gatewayProfileOptions} countryOptions={model.countryOptions} onCopy={model.copy} onSaveRouting={model.saveGatewayRouting} onReload={model.load} />}
