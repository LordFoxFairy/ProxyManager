...
export function GatewayPage({
  ...
  tunActual: { state: string; active: boolean; detail: string } | null;
    ...
    <>
      ...
      <section className="runtime-control-panel">
        ...
        <div className="runtime-control-grid">
          ...
          <button className={`runtime-toggle ${tunActual?.active ? 'active' : ''}`} title={tunActual?.detail} disabled={!runtimeStatus.capabilities.tun} onClick={() => toggleRuntime('tun')}><strong>TUN 模式</strong><span>{tunActual?.active ? '接口已创建' : runtimeStatus.features.tun === 'configured' ? (tunActual?.detail ?? '等待系统接口') : runtimeStatus.platform.tunDevice === 'permission-required' ? '点击后检查系统权限' : runtimeStatus.features.tun === 'failed' ? '启动失败' : runtimeStatus.tun === 'unsupported' ? '需 Mihomo + 系统权限' : '已关闭'}</span></button>
