    ...
    <>
      ...
      <section className="runtime-control-panel">
        ...
        <div className="runtime-control-grid">
          ...
          <button className={`runtime-toggle ${runtimeStatus.features.tun === 'active' ? 'active' : ''}`} disabled={!runtimeStatus.capabilities.tun} onClick={() => toggleRuntime('tun')}><strong>TUN 模式</strong><span>{runtimeStatus.features.tun === 'active' ? '已接管' : runtimeStatus.features.tun === 'configured' ? '已配置，等待系统路由' : runtimeStatus.features.tun === 'failed' ? '启动失败' : runtimeStatus.tun === 'unsupported' ? '需 Mihomo + 系统权限' : '已关闭'}</span></button>
